...
class Blockchain {
    ...
    async buildBlocks(peerList) {
        ...
        await Promise.all(
            peerList.map(peer => {
                return request({
                        "url": `http://${peer.ip}:${peer.port}/api/blocks?limit=${this.config.blocks}`,
        ...
        console.log("Processing blocks from", Object.keys(peerBlocks).length, "peers.");
        ...
        Object.keys(peerBlocks).forEach(peer => {
            ...
            {
                // Blocks differ!
                if (peerBlocks[peer][0].height == consensusPeer[0].height && peerBlocks[peer][0].blockSignature != consensusPeer[0].blockSignature) {
                    // Signatures differ! Take the smallest timestamp.
                    if (this.verifyBlock(peerBlocks[peer][0]) && peerBlocks[peer][0].timestamp < consensusPeer[0].timestamp) {
                        diffCount++;
                        console.log("Orphaned block found with ID", consensusPeer[0].id);
                        consensusPeer = peerBlocks[peer];
                    }
                } else if (peerBlocks[peer][0].height > consensusPeer[0].height) {
                    ...
                    diffCount++;
                    if (this.verifyBlock(peerBlocks[peer][0])) {
                        consensusPeer = peerBlocks[peer];
                    }
                }
            }
        ...
        else
        {
            ...
            if(!this.verifyChain()) {
                ...
                if(peering)
                    await this.buildBlocks(this.peers.getPeerList());
            } else {
                this.rebuilding = false;
                console.log("Latest height -", this._tail.height , "-", this._tail.id);
            }
    ...
    broadcastLatestBlock() {
        ...
        broadcastTo.forEach(peer => {
            const postReq = postBlock({ ip: peer.ip, port: peer.port});
    ...
    addBlock(block) {
        ...
        if(heightDiff > 1)
        {
            ...
            if (!this.rebuilding) {
                ...
                return this.buildBlocks(this.config.peers);
            }
        }
        ...
        if(block.previousBlock != latestBlock.id)
        ...
        console.log(`Adding new block. ID: ${block.id} | Height: ${block.height}`);
